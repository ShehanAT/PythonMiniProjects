    ...
    while not gameover:
        ...
        for i in range(game.height):
            for j in range(game.width):
                pygame.draw.rect(window, GREY, [game.startX + game.zoom * j, game.startY + game.zoom * i, game.zoom, game.zoom - 1], 1)
                if game.field[i][j] > 0:
                    pygame.draw.rect(window, colors[game.field[i][j]], 
                        [game.startX + game.zoom * j, game.startY + game.zoom * i, game.zoom - 2, game.zoom - 1])
        # draws Figure instance to game grid 
        ...
        text_gameover_1 = font1.render("Game Over", True, BLACK)
        text_gameover_2 = font1.render("Press ESC", True, BLACK)
...
# run the main function only if you run this file
