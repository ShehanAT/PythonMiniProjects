from os import kill
from snakeGame.main import game_loop
import pygame
import pygame_menu 
import random
import sys
...
def setup_collision_obj():
    new_collition_obj = [random.randrange(1, (display_width//10)) * 10, random.randrange(1, (display_height//10))* 10]
    return new_collition_obj
...
def show_start_screen():
    start_menu = pygame_menu.Menu(widht=display_width, height=display_height, title="Welcome to Snake Game!", theme=pygame_menu.themes.THEME_BLUE)
...
def game_loop():
    ...
    snake_body = [[display_width/2, display_height/2], [(display_width/2) - 10, display_height/2], [(display_width/2)-(2*10), display_height/2]]
    snake_width = 20 
    snake_height = 20 
    snake_speed = 5 
    snake_direction = "UP"
    ...
    while not gameExit:
        ...
        if isclose(snake_position[0], collision_obj_position[0], abs_tol=(snakewidth -10)) and isclose(snake_position[1], collision_obj_position[1], abs_tol=(snake_height - 10)):
        ...
        for pos in snake_body:
            pygame.draw.rect(win, white, pygame.Rect(pos[0], pos[1], snake_width/2, snake_height/2))
